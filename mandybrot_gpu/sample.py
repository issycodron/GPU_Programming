...
def area(real, imag, width, height, scale, max_iters):
    '''
    Sample the Mandelbrot set in the given region of the complex plane.
    '''
    ...
    kernel = cl.Program(
        ctx,
        """
        __kernel void mandelbrot(__global float* buffer, float start_real, float start_imag,
            uint width, float delta_real, float delta_imag, uint max_iters) {
